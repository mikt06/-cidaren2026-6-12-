...
一键自动完成词达人所有班级任务。支持全题型（拼写/填空/排序/卡片）。

**原理：空白试探法** — 提交空答案 → 服务端返回正确答案 → 二次提交得分。无需题库。
...
| `bash loop.sh` | 自动循环刷题，分数稳定后自动停止 |

## 注意事项

- ⚠️ 运行期间**不要用手机打开词达人**
- Token 过期（几小时到一天）后需重新抓取
- 遇到"权限不足"的任务自动跳过（如未购买的 CET4 课程）
- 已完成低分任务会自动重置重刷
- 极少数短提示拼写题可能无法自动作答（不影响大部分得分）
